...
**Rule: pass `--user-agent apify-claude-code-plugin/apify-actor-development` only on actor run commands (`apify run`). Do not add it to login, info, push, or other CLI commands.**
...
```bash
apify run --user-agent apify-claude-code-plugin/apify-actor-development     # Run Actor locally
